...
/** Matches a single G-code "word" — a letter immediately followed by a number — anywhere in a
 * line, with or without a separator before/after it. Real-world files (this app's own generator
 * included) don't reliably put whitespace between words on the same line (e.g. LightBurn happily
 * emits `Y-14.22S200F6000` as one run of characters), so tokenizing by whitespace would silently
 * corrupt every word but the first on such a line — this is why `parseGcodeProgram` uses this
 * instead of a plain `line.split(/\s+/)`. */
 ...
 *
 * Tokenizes each line into words via `WORD_PATTERN` (rather than substring-matching, e.g. `/G0/`)
 * so a word like `G90`/`G28` — sharing a leading digit with `G0` but modally unrelated to it — is
 * never mistaken for a motion command. Tracks X/Y across lines that only specify one axis, and
 * which motion mode and distance mode (`G90` absolute / `G91` relative) are active, all three
 * modally (carried over from the last line that set them until changed), so a file that spreads a
 * single logical move across several lines still renders correctly.
 *
 * G2/G3 (arcs) move the tracked cursor to their endpoint (so the next G0/G1 doesn't draw a bogus
 * long segment back to a stale position) but themselves produce no segment — this app's own
 * generator never emits them (see the "no curves" rule throughout the SVG import pipeline), and
 * correctly drawing one would need to interpret I/J/R parameters this parser doesn't read. */
export function parseGcodeProgram(content: string): GcodeSegment[] {
  ...
  for (const rawLine of content.split('\n')) {
    ...
    for (const [, rawLetter, rawValue] of line.matchAll(WORD_PATTERN)) {
      ...
      if (letter === 'G' && (value === 0 || value === 1)) {
        ...
        motion = null;
      } else if (letter === 'G' && value === 90) {
        absolute = true;
      } else if (letter === 'G' && value === 91) {
        absolute = false;
      } else if (letter === 'X') {
        targetX = value;
      } else if (letter === 'Y') {
    ...
    const nextX = targetX === null ? x : absolute ? targetX : x + targetX;
    const nextY = targetY === null ? y : absolute ? targetY : y + targetY;
